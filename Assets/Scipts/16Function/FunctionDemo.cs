...
- 내장함수 : 닷넷(c#)에서 제공, 유니티에서 제공하는 함수      //닷넷(C#) 기본 함수: Console.WriteLine("안녕"); 콘솔 출력 
...
void 함수이름()           //매개변수가 없는 함수(반환값 없음->Void)
{
    //하나 이상의 명령문...
}

void 함수이름(매개변수)   //매개변수 있는 함수(괄호안에 들어가면)
{
    //하나 이상의 명령문...
}
 
(데이터 타입) 함수이름(매개변수)   //반환값이 있는 함수  , 보이드없음(반환값이없다는건 보이드 라는 것)
{
    //하나 이상의 명령문...
    return (데이터 타입)변수;                               
}
